    ...
    fn parse(&self, header: &Header) -> Result<TimeZone, TzFileError> {
        ...
        for arr in self.local_time_types.chunks_exact(6) {
            ...
            let time_zone_designation = match self.time_zone_designations[char_index..].iter().position(|&c| c == b'\0') {
                Some(position) => Some(str::from_utf8(&self.time_zone_designations[char_index..char_index + position])?.into()),
...
mod test {
    ...
    fn test_v2_file() -> Result<(), TzError> {
        ...
        assert_eq!(*time_zone.find_local_time_type(-1156939200)?, LocalTimeType { ut_offset: -34200, is_dst: true, time_zone_designation: Some("HDT".into()) });
        assert_eq!(*time_zone.find_local_time_type(1546300800)?, LocalTimeType { ut_offset: -36000, is_dst: false, time_zone_designation: Some("HST".into()) });
    ...
    fn test_v3_file() -> Result<(), TzError> {
        ...
        let time_zone_result = TimeZone {
            ...
            extra_rule: Some(TransitionRule::Alternate {
                ...
                dst: LocalTimeType { ut_offset: 10800, is_dst: true, time_zone_designation: Some("IDT".into()) },
